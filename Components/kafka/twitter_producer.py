...
class StdOutListener(StreamListener):
  ...
  def on_data(self,data): 
    ...
    # self.producer.send(self.topic, key=tweet_data['id'], value=json.dumps(tweet_data).encode())
...
class Kafka_producer():
  def __init__(self):
    self.topic = "covid"
    # self.partitioner = RoundRobinPartitioner(partitions=[
    #     TopicPartition(topic=self.topic, partition=0),
    #     TopicPartition(topic=self.topic, partition=1)
    # ])
